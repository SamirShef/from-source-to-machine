...
Пришла пора проверить движок диагностики на нескольких реальных тестах, чтобы насладиться нашими стараниями
и увидеть вывод.
...
Предупреждение компилятора. Обратите внимание: функция `DiagCodeToIntegerCode` автоматически вычитает
смещение `WARN_CODE_START`, поэтому код предупреждения выводится как `W0000`, а внизу добавляется подсказка
`Note`.
