...
`TokenKind` --- это тип токена. `compiler/include/lexer/token_kind.h`:
...
ещё и движок диагностики (по ссылке, чтобы состояние движка изменялось). `compiler/include/lexer/lexer.h`:
