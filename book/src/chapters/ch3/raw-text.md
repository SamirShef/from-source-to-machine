...
То разница в подходах (список vs. поток токенов) окажется несущественной. Но как только в одном файле будет
...
он строит синтаксис для каждой синтаксической конструкции. Если ему понадобится пройти дальше (узнать новый
...
каких-то специальных символов (например, `$`, как в Pebble), то нужно научить лексер создавать валидный токен
и выкидывать ошибку. Для этого и существует `TokenKind::Unknown`.
