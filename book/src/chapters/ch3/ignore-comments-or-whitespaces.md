...
нужно пропускать пробельные символы и комментарии.
...
Лексер подумает, что паттерн `*/` на 4-й строке закрывает многострочный комментарий, открытый в 1-й строке.
Из-за этого в 5-й строке он сгенерирует два токена: `*` и `/`.
...
Из-за того, что мы не проверяем выход за пределы буфера (файла), цикл не остановится.
